  ...
  useEffect(() => {
    const uploadSnapshot = async () => {
      if (image && quizResultId) {
        ...
        await axios
          .post(
            `${import.meta.env.VITE_API_BASE_URL}/api/student/snapshot`,
            formData,
            {
              headers: {
  ...
  return (
    <Fragment>
      ...
      {localImage ? (
        <Fragment>
          {image && (
            ...
            >
              ...
              <img className="img-fluid" src={image} alt="Canvas as Image" />
            </div>
          )}
        </Fragment>
      ) : (
        <div
          className={
            `CameraResult position-relative` + (localImage ? "hasPhoto" : "")
          }
        >
          <canvas ref={photoRef}></canvas>
          <button
            className={`CameraPhotoClose ${localImage ? "d-block" : "d-none"}`}
            onClick={closePhoto}
          >
            CLOSE!
          </button>
